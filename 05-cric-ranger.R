...
Info = list(
   analysis_id    = paste0("cric_ranger: ", t_vars[1]),
...
#if (length(weight_col) ==1) task0$set_col_roles(Info$weight_col, roles = "weight")
...
mylearner = lrn("surv.ranger") #, type = "regression")
...
#prediction = lrn("surv.ranger")$train(task0, split$train)$   # learn using train data
...
# prediction$distr[1:3]$survival(5) # !!! Error: attempt to apply non-function
