...
# Define a graph for the pipeline
library(paradox)

# Define a function to apply splines transformation
apply_splines <- function(x) as.data.table(splines::ns(x, df = 3))

# Define the pipeline graph for the transformation
graph <- po("colapply", id = "apply_splines", applicator = apply_splines, affect_columns = selector_name(c("X1", "X2", "X3"))) %>>%
  learner
